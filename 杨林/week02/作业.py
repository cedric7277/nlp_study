...
"""
五维判断：x是一个5维向量，哪一维数字最大就属于第几类

5分类：输出概率分布，要分几类就输出几维的向量【0.1,0.1,0.1,0.5,0.2】
规则：最大元素在第k维 → 类别k
"""


类MultiClassficationModel(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        if y is not None:
            返回self.loss(y_pred, y)
        else:
            返回torch.softmax(y_pred, dim=1)
...
def main():
    ...
    # 选择优化器
    optim = torch.optim.Adam(model.参数(), lr=learning_rate)
日志 =[]
    # 创建训练集，正常任务是读取训练集
